...
class UserSerializerTests(TestCase):
    ...
    def test_view_user(self):
        ...
        with freeze_time(self.datetime):
            ...
            self.assertEqual(data["last_login"], self.datetime)
            self.assertEqual(data["date_joined"], self.datetime)
    ...
    def test_create_user(self):
        ...
        with freeze_time(self.datetime):
            ...
            serializer = UserSerializer(data=data, context={"request": self.request})
            ...
            self.assertEqual(
                user.last_login, datetime(2020, 11, 29, 0, 0, 0, 0, tzinfo=pytz.UTC)
            )
            self.assertEqual(
                user.date_joined, datetime(2020, 11, 29, 0, 0, 0, 0, tzinfo=pytz.UTC)
            )
